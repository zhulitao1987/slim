...
class emay_sms{
     ...
     * 网关地址
     */
    private $gwUrl = 'http://hprpt2.eucp.b2m.cn:8080/sdk/SDKService?wsdl';

    /**
     * 序列号,请通过亿美销售人员获取
     */
    private $serialNumber = '8SDK-EMY-6699-RKTLO';

    /**
     * 密码,请通过亿美销售人员获取
     */
    private $password = '353449';
    ...
    public function __construct(){
        $this->client = new Client($this->gwUrl, $this->serialNumber, $this->password, $this->sessionKey, $this->proxyhost, $this->proxyport, $this->proxyusername,
    ...
    //----------------------------------------------------------------------
    // 注:
    // 1. 下面是各接口的使用用例，eclient.php 还有每一个接口更详细的参数说明
    ...
    public function login()
    {
        ...
        if ($statusCode != null && $statusCode == "0") {
            ...
            outJson(-1, "登录成功, session key:" . $this->client->getSessionKey() . "<br/>");
        } else {
            //登录失败处理
            outJson(-1, "登录失败,返回:" . $statusCode);
        }
    }
     ...
     * 短信发送 用例
     */
    public function sendSMS($phone_arr,$content)
    {
        /**
         * 下面的代码将发送内容为 test 给 159xxxxxxxx 和 159xxxxxxxx
         * $client->sendSMS还有更多可用参数，请参考 eclient.php
         */
        if( !is_array($phone_arr) ){
            $phone_arr = array($phone_arr);
        }
        $statusCode = $this->client->sendSMS($phone_arr, $content);
        return $statusCode;
    }
     ...
     * 发送语音验证码 用例
     */
    public function sendVoice($phone_arr,$content)
    {
        /**
        ...
        $statusCode = $this->client->sendVoice($phone_arr, $content);
        return $statusCode;
    }
